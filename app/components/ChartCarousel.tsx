...
function labelData(data: Record<string, number>[], xLabels: string[]) {
  const step = Math.floor(data.length / (xLabels.length - 1))
  return data.map((d, i) => {
    const labelIndex = Math.round(i / step)
    const showLabel = i === 0 || i === data.length - 1 || i % step === 0
    return { ...d, x: showLabel ? xLabels[Math.min(labelIndex, xLabels.length - 1)] : '' }
  })
}
...
const slides = [
  {
    ...
    icon: '📈',
    data: labelData(usData, ['2011', '2014', '2017', '2020', '2023', '2026']),
    lines: [
  ...
  },
  {
    ...
    icon: '🌍',
    data: labelData(divData, ['Mar 23', 'Sep 23', 'Mar 24', 'Sep 24', 'Apr 26']),
    lines: [
  ...
  return (
    <section className="py-32" style={{ backgroundColor: '#f0f4ff' }}>
      <div className="max-w-7xl mx-auto px-6">
        ...
        >
          {/* Progress bar */}
          <div style={{ height: '3px', backgroundColor: 'rgba(37,99,235,0.08)' }}>
            <div style={{
              height: '100%',
              width: `${progress}%`,
                  ...
                  <AreaChart data={slide.data} margin={{ top: 10, right: 10, left: 0, bottom: 5 }}>
                    <defs>
                      {slide.lines.map((line) => (
                        <linearGradient key={line.key} id={`g-${line.key}-${displayActive}`} x1="0" y1="0" x2="0" y2="1">
                    ...
                    {slide.lines.map((line) => (
                      <Area
                        key={`${line.key}-${displayActive}`}
                        type="monotone"
                        dataKey={line.key}
                        name={line.name}
                        stroke={line.color}
                        strokeWidth={2}
                        fill={`url(#g-${line.key}-${displayActive})`}
                        dot={false}
